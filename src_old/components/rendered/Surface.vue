  ...
  <v-app id="inspire">
    ...
    <v-main class="grey lighten-3">
      <v-container class="mb-6">
        <div class="text-h3 pt-6">{{ word }}</div>
        <v-row>
          <v-col>
            <v-container>
              <v-card>
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    text
                    :href="'https://itwewina.altlab.app/word/' + word + '/'"
                    target="_blank"
                  >
                    <v-icon left>mdi-open-in-new</v-icon>View {{ word }} on itwêwina</v-btn
                  >
                </v-card-actions>
              </v-card>
            </v-container>
        ...
        <v-row>
          <v-col>
            <v-card-title class="pl-0">Surface form distribution for {{ word }}</v-card-title>
...
export default {
  ...
  mounted() {
    ...
    lexicon.lookup(this.word).then((itwewinaResults) => {
      console.log('itw', itwewinaResults)
      this.itwewinaResults = [itwewinaResults[0]]
    })

    lexicon.getSurfaceCount(this.word).then((surfaceCounts) => {
