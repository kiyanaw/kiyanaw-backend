...
describe('useWavesurferEvents', () => {
  ...
  describe('useEffect behavior', () => {
    ...
    it('should re-register listeners when transcriptionId changes', () => {
      ...
      expect(mockOn).toHaveBeenCalledTimes(8); // Initial call

      rerender({ id: 'id-2' });

      expect(wavesurferService.clearAllListeners).toHaveBeenCalledTimes(1);
      expect(mockOn).toHaveBeenCalledTimes(16); // Called 8 more times
    });
  ...
  describe('ready event handling', () => {
    it('should call playerStore.setLoadedAndReady(true) when ready event fires', () => {
      ...
      readyHandler({ duration: 120 }); // Emit with duration data

      // Check that the handler calls the correct store method
      expect(mockPlayerStore.setLoadedAndReady).toHaveBeenCalledWith(true);
      expect(mockPlayerStore.setDuration).toHaveBeenCalledWith(120);
    });
  });
